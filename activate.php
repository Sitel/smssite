...
$code = $_POST[T2];
 ...
 $check = mysql_query("SELECT activation FROM users WHERE username = '$username'");
$row = mysql_fetch_row($check);

if ($row[0] == $code)
{
mysql_query("UPDATE users SET active = 'yes' WHERE username = '$username'");
$_SESSION['active'] = "yes";
  ...
  // check to see if user was reffered by some affiliate
  $check = mysql_query("SELECT refferedby FROM users WHERE username = '$username'");
if(mysql_num_rows($check ) != 0) 
 { 
$row = mysql_fetch_row($check);
$affiliate = $row[0];
mysql_query("UPDATE affiliates SET registrations = registrations+1 WHERE username = '$affiliate'");
mysql_query("UPDATE users SET sms = sms+1 WHERE username = '$affiliate'");
	...
	<tr>
		...
		<p align="left" class="o" style="margin-right: 25px; margin-left:10px">
			<img border="0" src="images/arr2.jpg" width="10" height="7" alt="" style="vertical-align:middle;">
			 <? if ($_SESSION['active'] == 'yes') { echo "Your account has been activated."; } else { echo "Code error. Please try again"; } ?> <p style="margin-left: 5px; margin-right: 5px">
