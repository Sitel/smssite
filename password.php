...
if (!$error){
  $check = mysql_query("SELECT password FROM users WHERE username = '$username'");
	...
	if ($old != $row[0]) {$error[] = 'Old password not matched!'; };
} 
...
mysql_query("UPDATE users SET password = '$new1' WHERE username = '$usr'");
...
echo "			<p style=\"margin-left: 5px; margin-right: 5px\" align=\"center\" class=\"o\">&nbsp;			
			<a href=\"profile.php\">Go back</a></p><br><br>";
}
else
{
echo "			<p align=\"center\" class=\"o\" style=\"margin-left: 5px; margin-right: 5px; margin-bottom:0\">Password changed.</p><br><br><br><br><br>";
}
